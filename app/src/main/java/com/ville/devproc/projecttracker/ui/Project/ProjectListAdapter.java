...
public class ProjectListAdapter extends RecyclerView.Adapter<ProjectListAdapter.ProjectViewHolder> {
    ...
    public void deleteCheckedProjects() {
        ...
        if(mCheckToProjectMap.keySet().size() > 0) {
            ArrayList<Integer> idList = new ArrayList<>(mCheckToProjectMap.keySet());

            // Delete selected project timesheets from the Timesheet table.
            mDB.deleteAllProjectTimesheets(idList);

            // Delete selected projects from the ProjectWorker table
            mDB.deleteAllProjectWorker(idList);

            // Delete selected projects from the Project table
            isDeleteSuccess = mDB.deleteProjects(idList);
        }

        if( isDeleteSuccess ) {
            checkAllItems(false);
        } else {
            Snackbar.make(((Activity) mContext).findViewById(R.id.projectDeleteButton), "Removal unsuccessful.",
